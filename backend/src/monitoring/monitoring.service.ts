...
export class MonitoringService {
   ...
   */
  /**
   * The monitoring list. Ping monitors and their service checks — that is all
   * there is now.
   *
   * This method used to merge two monitoring subsystems: ICMP monitors from
   * `MonitorTarget` and SNMP devices from `NetworkDevice`, correlated by IP so
   * a box that was both pinged and SNMP-polled appeared once. It also folded
   * in port counts, open alerts and 24-hour syslog volume.
   *
   * All of that came from the NDM module, which has been removed. Nothing
   * writes to `NetworkDevice`, `Alert` or `SyslogEvent` any more, so querying
   * them would return empty sets and the merge would be an expensive way to
   * produce nothing.
   *
   * THE RESPONSE SHAPE IS DELIBERATELY UNCHANGED. `snmp`, `ports` and
   * `syslog` are still present and still `null`, and the summary still carries
   * the counters — now fixed at zero. The frontend already treats a null SNMP
   * half as "ping-only" and renders it correctly, so the API stays
   * backward-compatible and nothing has to be redeployed in lockstep.
   */
  async unifiedList(actor?: Actor) {
    const ids = await this.ownedIds(actor);

    const pings = await this.prisma.monitorTarget.findMany({
      where: ids ? { ownerId: { in: ids.length ? ids : [-1] } } : {},
      orderBy: [{ groupName: 'asc' }, { name: 'asc' }],
    });
    ...
    const rows = pings.map((p) => ({
      ...
      // Kept, always null — see the note above on backward compatibility.
    ...
    const summary = {
      total: rows.length,
      up: rows.filter((r) => r.enabled && r.ping.isUp === true).length,
      down: rows.filter((r) => r.enabled && r.ping.isUp === false).length,
      paused: rows.filter((r) => !r.enabled).length,
      // Retained at zero so existing clients keep parsing the same object.
      portsUp: 0, portsDown: 0, openAlerts: 0, syslog24h: 0,
      snmpDevices: 0,
