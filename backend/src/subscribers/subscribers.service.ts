...
if (!unpaid && data.username && data.password) {
  ...
  // Rate series (upload/download bytes per second) derived from
  // subscriber_traffic_sample - 10-minute MRTG-style buckets written by
  ...
  async getBandwidthHistory(username: string, minutes = 60) {
    try {
      const pg = this.radiusSync.getPgClient();
      if (!pg) return { samples: [], source: 'samples' };

      const cutoff = `NOW() - INTERVAL '${minutes} minutes'`;
      const res = await pg.query(`
        SELECT s.ts, s.in_bytes, s.out_bytes
          FROM subscriber_traffic_sample s
          JOIN "Subscriber" sub ON sub.id = s.subscriber_id
         WHERE sub.username = $1 AND s.ts >= ${cutoff}
         ORDER BY s.ts ASC
      `, [username]);

      // Rate (bytes/sec) between consecutive samples, exactly like MRTG /
      // nas_traffic_sample. Only pairs with a real inter-sample gap produce
      // a point; counter resets clamp to zero instead of a negative spike.
      const raw = res.rows;
      const samples: any[] = [];
      let prevUp = 0;
      let prevDown = 0;
      ...
      for (const row of raw) {
        ...
        if (prevTime && t.getTime() !== prevTime.getTime()) {
          ...
          const upBps = dt > 0 ? Math.max(0, (Number(row.in_bytes) - prevUp) / dt) : 0;
          const downBps = dt > 0 ? Math.max(0, (Number(row.out_bytes) - prevDown) / dt) : 0;
          samples.push({
            timestamp: t.toISOString(),
            uploadBps: Math.round(upBps),
            downloadBps: Math.round(downBps),
            uploadBytes: Number(row.in_bytes),
            downloadBytes: Number(row.out_bytes),
          });
        }
        prevUp = Number(row.in_bytes);
        prevDown = Number(row.out_bytes);
        prevTime = t;
      }

      return { samples, source: 'samples' };
    } catch (error: any) {
