...
export default function LoginPage() {
  ...
  const handleDemo = async () => {
    setLoading(true);
    setMessage("Creating a NEW demo account…");
    setDemoCredentials(null);
    try {
      // Always create a fresh demo account. Never reuse the shared demo endpoint.
      const r = await fetch(`${API_BASE}/demo/create`, { method:"POST", headers:{"Content-Type":"application/json"}, cache:"no-store" });
      let d:any = {};
      try { d = await r.json(); } catch {}
      const loginEmail = d.email || d.username || d.user?.email || d.user?.username;
      const loginPassword = d.password;
      if (!r.ok || !loginEmail || !loginPassword) throw new Error(d?.message || "The demo service did not return usable credentials");
      const c={email:String(loginEmail),password:String(loginPassword)};
      setEmail(c.email);
      setPassword(c.password);
      setCode("");
      setNeeds2fa(false);
      setDemoCredentials(c);
      setMessage("✅ NEW demo account created. Credentials are shown below.");
    } catch (e:any) {
      setMessage(`❌ ${e?.message || "Could not create a demo account right now. Please try again."}`);
    } finally { setLoading(false); }
  };
  ...
  return <div style={{background:"#080b12",minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",padding:20,fontFamily:"'Segoe UI',system-ui,sans-serif"}}>
    <div style={{display:"flex",width:900,maxWidth:"100%",minHeight:520,borderRadius:24,overflow:"hidden",border:"1px solid rgba(255,255,255,.08)",boxShadow:"0 25px 50px -12px rgba(0,0,0,.5)"}}>
      ...
      <label style={{color:"#94a3b8",fontSize:11,marginBottom:7}}>{t("Email Address")}</label><input type="email" value={email} onChange={e=>setEmail(e.target.value)} onKeyDown={handleKeyPress} autoComplete="username" style={{width:"100%",boxSizing:"border-box",background:"rgba(255,255,255,.04)",border:"1px solid rgba(255,255,255,.08)",borderRadius:10,padding:"12px 14px",color:"#fff",marginBottom:16}}/>
